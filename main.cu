...
int main(int argc, char** argv)
{
    ...
    for (int power = 5; power <= 15; ++power) 
    {
        ...
        if (N % 2 == 0) {
            ...
            config_f2.grid_dim = dim3{(N/2 + TILE - 1) / TILE, (N/2 + TILE - 1) / TILE};
            results.push_back(RunTest("Float2", TransposeVectorized<TILE, float2>, config_f2));
        }

        if (N % 4 == 0) {
            BenchmarkConfig config_f4 = config;
            config_f4.grid_dim = dim3{(N/4 + TILE - 1) / TILE, (N/4 + TILE - 1) / TILE};
            results.push_back(RunTest("Float4", TransposeVectorized<TILE, float4>, config_f4));
        }
    }
